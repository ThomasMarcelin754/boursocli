# boursobank — playbook de release Homebrew
...
helper + ce playbook + le template de formule (`Formula/boursobank.rb`).
...
- Un repo tap `thomasmarcelin754/homebrew-tap` existe (un repo GitHub *séparé*,
  nommé `homebrew-tap`) avec `Formula/boursobank.rb` initialisé depuis le
  template `Formula/boursobank.rb` de ce dépôt.
...
Éditer `../homebrew-tap/Formula/boursobank.rb` :
- mettre `version "X.Y.Z"`
- mettre `url "https://github.com/thomasmarcelin754/boursobank/archive/refs/tags/vX.Y.Z.tar.gz"`
- coller `sha256 "…"`

```sh
git -C ../homebrew-tap commit -am "boursobank vX.Y.Z"
git -C ../homebrew-tap push origin main
```

## 3) Vérifier l'installation
```sh
brew uninstall boursobank || true
brew untap thomasmarcelin754/tap || true
brew tap thomasmarcelin754/tap
brew install thomasmarcelin754/tap/boursobank
brew test thomasmarcelin754/tap/boursobank
boursobank --version
```
